    ...
    >
      <div className="space-y-4">
        <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-50 p-3.5 text-xs text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          ...
          <p>
            You are about to roll back active production traffic for <strong>{deployment.project_name}</strong> to release <strong>{deployment.id}</strong> ({deployment.commit_hash}).
          </p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs space-y-1.5 font-mono dark:border-slate-800 dark:bg-slate-950/80">
          <div className="flex justify-between text-slate-500 dark:text-slate-400">
            ...
            <span className="text-slate-900 dark:text-white font-medium">{deployment.commit_hash}</span>
          </div>
          <div className="flex justify-between text-slate-500 dark:text-slate-400">
            <span>Commit Message:</span>
            <span className="text-slate-900 dark:text-white font-medium truncate max-w-[180px]">{deployment.commit_message}</span>
          </div>
          <div className="flex justify-between text-slate-500 dark:text-slate-400">
            <span>Branch:</span>
