  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-indigo-600">Tuesday, July 29</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">
            Good morning, Jane
          </h1>
          <p className="mt-2 text-slate-500">Here is what is happening across your projects.</p>
        </div>
        <button
          type="button"
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            <h2 className="font-semibold text-slate-950">Recent deployments</h2>
            <p className="mt-0.5 text-sm text-slate-500">Your latest production activity.</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {deployments.map(([project, environment, started, status]) => (
                <tr className="border-t" key={`${project}-${started}`}>
                  <td className="px-5 py-4 font-medium text-slate-900">{project}</td>
                  <td className="px-5 py-4 text-slate-600">{environment}</td>
                  <td className="px-5 py-4 text-slate-600">{started}</td>
